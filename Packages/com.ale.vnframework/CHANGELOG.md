...
- **`FastForwardTypewriterRate`**（组件上是 `fastForwardTypewriterRate`），**默认 30 倍**：
  快进时台词打字机专用的倍率（字速与标点停顿）。原本的 `FastForwardRate`（默认 5 倍）
  从此只管补间、延时、角色 Animator、粒子与语音，**不再管打字机**。
  分开是因为快进时台词是唯一需要「一眼扫过」的东西，而演出跟着 30 倍走只会糊成一片。

### 变更

- **快进时的观感会变**：此前打字机与演出同为 5 倍，现在文字快得多。
  想要旧表现，把 `fastForwardTypewriterRate` 也调成 5 即可。
- 「快进与速度档取 `Max`」的取舍不变，只是基准换成了新的那条倍率：
  默认值下 `Max(3, 30)` 仍等价于覆盖，3 档速按下快进不会拿到 90 倍。

### 存档兼容

- `VnPlaybackSettingsData` 随之多一个字段，**1.6.2 及更早的存档可以直接读**。
  载入方把**非正数当作「字段缺失」**，保留组件上配好的默认值——而不是照搬兄弟字段的
  `Mathf.Max(1f, ...)` 夹成 1。夹成 1 会让快进时打字机完全不提速，
  表现为「按住快进，演出快了、字还是一个个蹦」，是那种不报错、只能靠肉眼发现的降级。
  JsonUtility 反序列化缺字段时保留字段初始值 30，本就无碍；这条防的是零初始化的自定义二进制格式。
...
- **读档后播放控制按钮的图标停在旧状态。** `LoadSaveData` / `ResetAll` 按 `ISaveable` 契约
  不触发变更事件，于是设置已经写进控制器、按钮却没人通知——现象是「读了档，自动播放明明是关的，
  按钮还亮着」。这两个方法现在会自动补一次 `Playback.NotifyStateChanged()`。
  契约里「不触发变更事件」针对的是**玩法**事件（避免读档层层级联），而按钮换图是纯视图刷新、
  不会回流进任何玩法逻辑，故作为例外放行。直接调底层的 `Playback.ApplySettings()` /
  `ResetToDefaults()` 仍不发事件，需要自己刷。
- **`VnPlaybackButton` 的订阅不再可能一次建不成就永久失效。** 此前订阅只在 `OnEnable` 建立，
  而按钮条是字幕面板的**兄弟节点**、不随对话开关激活，`OnEnable` 整局只跑一次——
  若那一刻 `VnStoryManager.Instance` 尚未就绪（这套 UI 由 Dialogue System 运行时实例化，
  时机不由本包决定），订阅就建不起来，该按钮此后再也不会刷新，且**不报任何错**。
  现在 `Start`（必定晚于场上所有 `Awake`）会再绑一次，`Refresh()` 每次也都先确认订阅
  仍挂在当前控制器上——后者顺带覆盖「管理器被重建、而这套 UI 活了下来」的情形，
  但那需要一次 `Refresh` 作为触发（例如点一下该按钮），不是无条件自愈。
